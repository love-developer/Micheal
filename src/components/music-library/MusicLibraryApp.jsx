...
const UploadMusicModal = ({ type, setType, onClose }) => (
  <Overlay>
    <div className='w-full max-w-[390px] rounded-[10px] bg-white p-4'>
      ...
      {type === 'video' && (
        <div className='mt-2 rounded border border-[#e5e7ec] bg-[#f8f9fb] p-2 text-xs'>
          <div className='mb-2'>
            <div className='mb-1 flex justify-between'><span>Production Quality</span><span>3/5</span></div>
            <div className='h-1.5 rounded-full bg-[#f5debf]'><div className='h-1.5 w-[45%] rounded-full bg-[#ff9800]' /></div>
          </div>
          <div>
            <div className='mb-1 flex justify-between'><span>Content Rating</span><span>3/5</span></div>
            <div className='h-1.5 rounded-full bg-[#f5debf]'><div className='h-1.5 w-[40%] rounded-full bg-[#ff9800]' /></div>
          </div>
        </div>
      )}

      <div className='mt-3 grid grid-cols-2 gap-2'>
        <button type='button' onClick={onClose} className='h-9 rounded border border-[#9ba3b4] text-xs text-[#566074]'>Cancel</button>
        <button type='button' onClick={onClose} className='h-9 rounded bg-[#ff9800] text-xs text-white'>{type === 'song' ? 'Upload Song' : 'Upload Video'}</button>
      </div>
    {/* </div> */}
  </Overlay>
)
...
const GoLiveModal = ({ onClose }) => (
  <Overlay>
    <div className='w-full max-w-[460px] rounded-[10px] bg-white p-4 text-xs'>
      ...
      <div className='mt-3 grid grid-cols-2 gap-2'><button type='button' onClick={onClose} className='h-9 rounded border border-[#9ba3b4] text-[#566074]'>Cancel</button><button type='button' onClick={onClose} className='h-9 rounded bg-[#ff9800] text-white'>Schedule Event</button></div>
  </Overlay>
)
